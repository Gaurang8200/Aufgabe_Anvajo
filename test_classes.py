...
def test_properties():
    ...
    '''pytest.fail("Remove this when you got an explanation.")
       The first time .hungry is accessed, _hungry is None, so it is set to random.choice([True, False]).
       Since _hungry is now assigned, its value does not change on future calls.
       This ensures that once a Cat object is created, it stays hungry or not.'''
